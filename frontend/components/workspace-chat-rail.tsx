...
export function WorkspaceChatRail({ compact = false, initialMessage }: { compact?: boolean; initialMessage?: string }) {
  ...
  const message = storedPrompt || initialMessage?.trim() || DEFAULT_MESSAGE;
  ...
  return (
    <aside className={`flex min-h-[calc(100vh-72px)] flex-col border-r border-white/10 bg-[#090b0e] ${compact ? "w-full" : "w-full lg:w-[39%]"}`}>
      ...
      <div className="border-t border-white/10 p-4"><div className="flex items-center rounded-2xl border border-white/10 bg-[#111317] p-2"><input value={message} onChange={(event) => setStoredPrompt(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") submitMessage(); }} className="min-w-0 flex-1 bg-transparent px-3 text-xs text-slate-300 outline-none placeholder:text-slate-700" placeholder="Ask about satellite change detection..." /><button type="button" onClick={submitMessage} className="grid h-9 w-9 place-items-center rounded-xl bg-white text-black" aria-label="Open analysis workspace"><Send className="h-4 w-4" /></button></div><div className="mt-3 flex items-center gap-4 text-slate-700"><button type="button" aria-label="Copy query" title="Copy query" onClick={() => { void navigator.clipboard?.writeText(message); setFeedback("Copied"); }}><Copy className="h-3.5 w-3.5" /></button><button type="button" aria-label="Helpful" title="Helpful" onClick={() => setFeedback("Thanks for the feedback") }><ThumbsUp className="h-3.5 w-3.5" /></button><button type="button" aria-label="Not helpful" title="Not helpful" onClick={() => setFeedback("Feedback recorded") }><ThumbsDown className="h-3.5 w-3.5" /></button>{feedback ? <span className="text-[10px] text-slate-500">{feedback}</span> : null}</div></div>
    </aside>
  );
}

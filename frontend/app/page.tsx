  ...
  return (
    <main className="garuda-frame relative flex min-h-screen items-center justify-center overflow-hidden px-5 text-white">
      ...
      <section className="relative z-10 w-full max-w-[760px] text-center">
        ...
        <form className="mx-auto mt-12 flex max-w-[620px] items-center rounded-[22px] border border-white/10 bg-[#0e1116] p-2 shadow-2xl shadow-black/30" onSubmit={(event) => { event.preventDefault(); if (prompt.trim()) router.push(`/map-view?prompt=${encodeURIComponent(prompt.trim())}`); }}><input value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="Analyze a place or ask a satellite question..." className="min-w-0 flex-1 bg-transparent px-5 text-sm text-white outline-none placeholder:text-slate-600" /><button type="submit" className="flex h-11 items-center gap-2 rounded-2xl bg-white px-5 text-sm font-medium text-black transition hover:bg-slate-200"><Send className="h-4 w-4" /> Analyze</button></form>

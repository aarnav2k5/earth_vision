...
import { ChangeHighlightComparison } from "@/components/change-highlight-comparison";
...
export default function AnalyticsPage() {
  ...
  return <div className="flex min-h-[calc(100vh-72px)] flex-col lg:flex-row"><WorkspaceChatRail /><section className="min-w-0 flex-1 overflow-auto bg-[#0b0e12] p-5 lg:p-8">
    ...
    {activeTab === "satellite" ? <div className="mt-4 space-y-4">{analysis ? <><div className="flex items-center gap-2 text-sm font-medium"><SatelliteIcon className="h-4 w-4 text-purple-300" /> Selected AOI-clipped Sentinel-2 scenes</div><div className="flex flex-wrap gap-4 rounded-2xl border border-white/10 bg-[#101318] p-4 text-xs text-slate-400"><span>Before: {analysis.before_acquired}</span><span>After: {analysis.after_acquired}</span><span>Cloud: {analysis.before_cloud_cover}% / {analysis.after_cloud_cover}%</span><span>OpenCV change: {analysis.metrics.opencv_change_percent ?? 0}% across {analysis.metrics.opencv_change_regions ?? 0} regions</span></div><div className="grid gap-4 lg:grid-cols-2">{[["Before", analysis.before_preview_url], ["After", analysis.after_preview_url]].map(([label, url]) => <div key={label} className="overflow-hidden rounded-2xl border border-white/10 bg-[#101318]"><div className="border-b border-white/10 px-4 py-3 text-xs font-medium text-slate-200">{label} · selected polygon area</div>{url ? <img src={url} alt={`${label} AOI-clipped satellite image`} className="h-64 w-full object-contain bg-[#0f1720]" /> : <div className="map-surface flex h-64 items-center justify-center text-xs text-slate-400">Preview unavailable</div>}</div>)}</div>{analysis.after_preview_url ? <ChangeHighlightComparison afterUrl={analysis.after_preview_url} contours={analysis.change_contours ?? []} masks={[{ name: "OpenCV image changes", values: analysis.opencv_change_mask ?? [], color: "#f97316" }, { name: "Vegetation", values: analysis.vegetation_change_mask, color: "#34d399" }, { name: "Water", values: analysis.water_change_mask, color: "#22d3ee" }, { name: "Built-surface", values: analysis.urban_change_mask, color: "#60a5fa" }]} /> : <EmptyTab text="AOI preview imagery is unavailable for this analysis, so change highlights cannot be projected." />}</> : <EmptyTab text="Run an analysis to load the selected AOI imagery and change highlights." />}</div> : null}
